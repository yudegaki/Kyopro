...
void dijkstra(ll st){
    ...
    while(!que.empty()){
        ...
        rep(i,G[tp.second].size()){
            pl next = G[tp.second][i];
            if(dist[tp.second] + next.first < dist[next.second]){
                dist[next.second] = dist[tp.second] + next.first;
